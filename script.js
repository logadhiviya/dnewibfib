...
function switchLoginTab(tab){
  ...
  document.querySelectorAll('.login-tab').forEach((t,i)=>t.classList.toggle('active',(tab==='visitor'&&i===0)||(tab==='admin'&&i===1)));
}
function loginVisitor(){
  const name=document.getElementById('visitorName').value.trim(),code=document.getElementById('visitorCode').value.trim(),err=document.getElementById('visitorError');
  if(!name){err.textContent='>> Please enter your name.';return;}
  if(code!==store.visitorCode){err.textContent='>> Invalid access code. Try VISITOR2026';return;}
  store.currentUser={type:'visitor',name};
  document.getElementById('navBadge').textContent='👤 '+name;
  document.getElementById('navBadge').className='nav-user-badge';
  document.querySelectorAll('.counter-num').forEach(el=>el.textContent='0');
...
function logout(){
  store.currentUser=null;portfolioReady=false;showPage('loginPage');
  ['visitorName','visitorCode','adminUser','adminPass'].forEach(id=>document.getElementById(id).value='');
  ['visitorError','adminError'].forEach(id=>document.getElementById(id).textContent='');
}
function switchToPortfolio(){
  document.getElementById('navBadge').textContent='⚙ ADMIN';
  document.getElementById('navBadge').className='nav-admin-badge';
  showPage('portfolioPage');
}
